    ...
    if (strcmp(response, "yes") == 0) {
        *answers = realloc(*answers, (*answersSize + 1)*sizeof(Node*));
        ...
        if(node->no != NULL) {
            ...
            printf("Не угадал :( Расскажите, что это было?\n");
            char newObject[100];
...
Node* parseNode(const char* line) {
    ...
    char* question = (char*) malloc((questionEnd - questionStart + 1) * sizeof(char));
    memcpy(question, questionStart, questionEnd - questionStart);
    question[questionEnd - questionStart] = '\0';
